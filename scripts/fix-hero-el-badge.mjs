/**
 * Replace Shell-tab molecule with a finished EL icon badge
 * (solid weight like the original molecule, complete letters).
 */
import fs from "fs";
...
// Finished square icon: soft plate + complete gradient EL (matches site logo)
const size = 128; // draw hi-res then downscale
const iconSvg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
  <defs>
    <linearGradient id="el" x1="16" y1="112" x2="112" y2="16" gradientUnits="userSpaceOnUse">
      <stop stop-color="#5EEAD4"/>
      <stop offset="0.55" stop-color="#38BDF8"/>
      <stop offset="1" stop-color="#2563EB"/>
    </linearGradient>
  </defs>
  <!-- no dark plate — transparent, letters only, full EL -->
  <g transform="translate(14,28) skewX(-10)" fill="url(#el)">
    <!-- E -->
    <rect x="0" y="0" width="42" height="12" rx="2.5"/>
    <rect x="0" y="26" width="34" height="12" rx="2.5"/>
    <rect x="0" y="52" width="42" height="12" rx="2.5"/>
    <!-- L complete -->
    <rect x="52" y="0" width="12" height="64" rx="2.5"/>
    <rect x="52" y="52" width="42" height="12" rx="2.5"/>
  </g>
</svg>`;

const hi = await sharp(Buffer.from(iconSvg)).png().toBuffer();
const side = Math.min(bw, bh) - 4;
...
const cover1 = await sharp({
  ...
  .png()
  .toBuffer();
const side1 = Math.min(bw1, bh1) - 4;
...
console.log("done", { bw, bh, side, v: V });
