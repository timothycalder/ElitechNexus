/**
 * Refine Cloud & DevOps + Security tiles with clearer engineering photos.
 */
import fs from "fs";
...
const cloudSrc = path.join(POOL, "cloud-devops-v2.jpg");
const platformSrc = path.join(POOL, "platform-skills.jpg");
const securitySrc = path.join(POOL, "security-v2.jpg");

// Dual monitors / ops engineer desk (cloud & devops work)
await download(
  "https://images.unsplash.com/photo-1518432031352-d6fc5c10da5a?auto=format&fit=crop&w=1200&q=85",
  cloudSrc
);
// Secure coding / lock concept on laptop (security)
await download(
...
await sharp(cloudSrc).resize(900, 560, { fit: "cover" }).png().toFile(path.join(SVC, "fde.png"));
await sharp(securitySrc).resize(900, 560, { fit: "cover" }).png().toFile(path.join(SVC, "security.png"));
